...
argument-hint: "One or more PR/MR URLs (space- or newline-separated). If a URL is invalid or does not match a supported platform pattern, respond with an error specifying the issue and stop. Actions: review (default), summarise, diff, fix comment: <text>, checkout. Re-passing same URL → follow-up mode (check threads, resolve or insist, approve if all clear)."
---
