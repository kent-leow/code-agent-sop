...
argument-hint: "Leave blank to only create missing SNAPSHOT.md files. Or describe what to change (e.g. 'add a Dependencies section')."
---
...
- LOOP: each in Missing list
  ...
  - DO: read build descriptor (`package.json`, `build.gradle`, `pom.xml`, etc.) for stack/scripts
  - DO: write `SNAPSHOT.md` per Structure below
  - IF: no README or build descriptor → write minimal: `Purpose: Unknown — no README or build descriptor found.`

## Phase 3B — Update Existing

- LOOP: each in Existing list
  - DO: read current `SNAPSHOT.md`
  - DO: apply change consistently (same position, rename everywhere, remove everywhere)
